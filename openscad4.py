...
def ip_sol2sol(sol1,sol2,n=0,triangulation_type=0):
    """
...
def closest_points_between_two_lines(l1,l2):
    ...
    if (isnan(t)& isnan(s))|(isinf(t)& isnan(s))|(isnan(t)& isinf(s))|(isinf(t)& isinf(s)):
...
#         b1=triangulate_solid_openx(sol2)
#     l1=[]
#     a,b=a_(a1).min(1),a_(a1).max(1)
#     c,d=a_(b1).min(1),a_(b1).max(1)
#     for i in range(len(a1)):
#         l1.append(a_(b1)[~((a[:,0][i]>d[:,0])+(b[:,0][i]<c[:,0])+ \
#         (a[:,1][i]>d[:,1])+(b[:,1][i]<c[:,1])+ \
#         (a[:,2][i]>d[:,2])+(b[:,2][i]<c[:,2]))].tolist())
...
#                 x1=triangle_triangle_intersection(a1[i],l1[i][j])
#                 if x1!=[]:
#                     l2.append(x1)
#     return l2
...
#     for i in range(len(a2)):
#         a,b,c=a2[i]
#         v0,v1,v2=b2[:,0],b2[:,1],b2[:,2]
#         n1=cross(b-a,c-a)
#         n1=n1/norm(n1)
#         d1=(einsum('j,ij->i',n1, uv(v0-a)))
#         d2=(einsum('j,ij->i',n1, uv(v1-a)))
#         d3=(einsum('j,ij->i',n1, uv(v2-a)))
#         d=a_([d1,d2,d3]).transpose(1,0)
#         c2=b2[~((d>0).all(1) | (d<0).all(1))]
#         mn1,mx1=a2[i].min(0),a2[i].max(0)
#         mn2,mx2=c2.min(1),c2.max(1)
#         c3=c2[~((mn1>mx2)+(mx1<mn2)).any(1)]
#         if c3.size!=0:
#             for j in range(len(c3)):
#                 x1=triangle_triangle_intersection(a2[i],c3[j])
#                 if x1!=[]:
#                     l1.append(x1)
#     return l1
...
#     c,d=a_(b1).min(1),a_(b1).max(1)
#     for i in range(len(a1)):
#         l1.append(b1[~((a[:,0][i]>d[:,0])+(b[:,0][i]<c[:,0])+ \
#         (a[:,1][i]>d[:,1])+(b[:,1][i]<c[:,1])+ \
#         (a[:,2][i]>d[:,2])+(b[:,2][i]<c[:,2]))].tolist())
...
# def two_surface_intersection(surf1,surf2):
#     a1=a_(triangulate_surface(surf1))
#     b1=a_(triangulate_surface(surf2))
#     l1=[]
#     a,b=a_(a1).min(1),a_(a1).max(1)
#     c,d=a_(b1).min(1),a_(b1).max(1)
#     for i in range(len(a1)):
...
def two_tri_intersection(a1,b1):
...
def plos_v_tri(s2,l1,v1,dist=100000,unidirection=0):
    
    return psos_v_tri(s2,[l1],v1,dist=dist,unidirection=unidirection)[0]

def psos_v_tri(s2,s3,v1,dist=100000,unidirection=0):
...
def sweep_sec2path(section2d,path3d,mirror=0,orientation=0,closed_loop=0):
    """
sweep a 2d section to a path, either closed or open
This function is similar to path_extrude_open and path_extrude_closed but is more
accurate, but a little slower.
mirror: set parameter to either '0' or '1'
orientation: set parameter to any value 0 , 1 , 2, 3
closed_loop: if the path is closed set this to '1' or else if path is open set it to '0'

example:
sec=corner_radius(pts1([[0,0,.2],[3,0,.2],[0,2,1],[-3,0,1]]),10)
path=helix(10,3,4,5.01)
sol=sweep_sec2path(sec,path,mirror=1,orientation=3,closed_loop=0)
fo(f'''
{swp(sol)}
color("blue")p_line3d({path},.1);
color("cyan") p_line3dc({sec},.2);
    ''')
...
def npos(surface,point,triangulation_type=0): # nearest point projection on surface
    if triangulation_type==0:  # example: cylinder with top and bottom triangulated
        tx=a_(triangulate_solid_open(surface))
    elif triangulation_type==1: # example: cylinder with top and bottom without triangulation
        tx=a_(triangulate_solid_openx(surface))
    ...
    px.append(p0[dc10])
    px=concatenate(px)
    py=px[norm(px-p3,axis=1).argmin()].tolist()
    return py

def nlos(surface,line,triangulation_type=0): # nearest line projection on surface
    if triangulation_type==0:  # example: cylinder with top and bottom triangulated
        tx=a_(triangulate_solid_open(surface))
    elif triangulation_type==1: # example: cylinder with top and bottom without triangulation
        tx=a_(triangulate_solid_openx(surface))
...
#     lx=[]
#     for i in range(len(a1)):
#         e=b1[~((a1[i]>d).any(1)|(a1[i]<c).any(1))]
#         if l_(e)==[]:
#             a1[i]=a_(npos(surface,a1[i],triangulation_type))
#             e=b1[~((a1[i]>d).any(1)|(a1[i]<c).any(1))]
#             lx.append(e)
#         else:
#             lx.append(e)
...
#                 px=ppos([lx[i][j]],a1[i],nv(lx[i][j]),0,1)
#                 x1=point_in_triangle(a_(px),a_(lx[i][j]))
#                 if x1==1:
#                     ly.append(l_(lx[i][j]))
#                     lz.append(i)
#                     break
#     dec=arange(len(lx))[~(arange(len(lx))[:,None]==a_(lz)[None,:]).any(1)]
#     for i in dec:
...
def o_3d(l1,s1,r=1,triangulation_type=0,outside=1):
    ...
    """
    l2=ipt(s1,l1,triangulation_type)
    l2=a_([nv(p) for p in l2])*-1
    l3=i_p_t_o(l1)
    l4=a_(cross(l3,l2))
    l4=l4/norm(l4,axis=1).reshape(-1,1)
    l5=l_(a_(l1)+l4*r)
...
def i_p_n(l1,s1,triangulation_type=0): # intersection point normals
    tx=ipt(s1,l1,triangulation_type=triangulation_type)
    p0,p1,p2=a_(tx)[:,0],a_(tx)[:,1],a_(tx)[:,2]
    v1=p1-p0
    v2=p2-p0
    vx=cross(v1,v2)
...
def point_near_triangle(triangle,point):
    p0,p1,p2=a_(triangle)
    p3=a_(point)
    n1=cross(p1-p0,p2-p0)
    n1=n1/norm(n1)
    d1=n1@(p3-p0)
    ...
    n3=cross(p1-p4,p2-p4)
    n4=cross(p2-p4,p0-p4)
    t1=((p4-p0)@(p1-p0))/((p1-p0)@(p1-p0))
    t2=((p4-p1)@(p2-p1))/((p2-p1)@(p2-p1))
    t3=((p4-p2)@(p0-p2))/((p0-p2)@(p0-p2))
    if ((n1@n2)>0)&((n1@n3)>0)&((n1@n4)>0):
        px=p4
    elif ((n1@n2)<0)&(0<=t1<=1):
        px=p0+(p1-p0)*t1
    elif ((n1@n2)<0)&(t1<0):
